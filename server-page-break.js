pageBreak=()=>{
    ...
    componentsWithY.forEach((componentData, index) => {
      ...
      if (this.isHeightInRange(componentData.yTop, componentData.yBottom, obj, componentData.element, componentsWithY, index)) {
        console.log(`Height ${givenHeight}px is within the range of Component ${index + 1}`);
      } else {
        
        console.log(`Height ${givenHeight}px is not within the range of Component ${index + 1}`);
      }
...
isHeightInRange = (yTop, yBottom, obj, component, index) => {
    ...
    // if(yBottom > obj.givenHeight){
      console.log("yTop",yTop,  "yBottoom", yBottom, "total margin", obj.marginAdded, "pageHeight", obj.givenHeight);
